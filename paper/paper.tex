...
\title{Resolvable Ambiguity}
%% \thanks{This project is financially supported by the Swedish Foundation for Strategic Research (FFL15-0032)} TODO: figure out where to include this
\author{Viktor Palmkvist}
\email{vipa@kth.se}
\orcid{0000-0003-0669-4085}
...
\affiliation{%
  ...
  \streetaddress{100 44}
  \city{Stockholm}
  \state{Sweden}
  }

\renewcommand{\shortauthors}{V. Palmkvist et al.}
...
\section{Notation}\label{sec:prel}
...
% TODO: this actually refers to ``Concrete Syntax'' and ``Abstract Syntax Trees'', i.e., not to ``Concrete Syntax Trees''. Not sure how to clarify that
\section{Describing Concrete Syntax and Abstract Syntax Trees}\label{sec:grammars}

This section describes the grammars we use to describe the concrete
syntax and corresponding abstract syntax trees (ASTs) of programs. We
start with context-free grammars and modify them step by step. The
purpose of these modifications is two-fold: we require explicit
descriptions of ASTs, and we require these ASTs to discard some
information from the original parsed program. The discarded
information can then be used for disambiguation, which we describe in
more detail in Section~\ref{sec:tree-language}.

We use the following small language with addition, multiplication,
lists, let-expressions, and numeric literals as a running example:
...
\begin{center}
  \begin{minipage}[t]{.5\columnwidth}
    \begin{tabular}{@{}l@{\quad}l@{}}
      $E$\quad$->$ & $E$ \texttt{'+'} $E$ \\
      $E$\quad$->$ & $E$ \texttt{'*'} $E$ \\
      $E$\quad$->$ & \texttt{'['} $E_l$ \texttt{']'} \\
      $E$\quad$->$ & \texttt{'let'} \texttt{Ident} \\ % TODO: Ident stands for any identifier, which means that 'Ident' is confusing, but I'm not entirely sure how to present it
      & \texttt{'='} $E$ \texttt{'in'} $E$ \\
      $E$\quad$->$ & \texttt{Int} \\
    \end{tabular}
  \end{minipage}
...
\noindent Monospace font, \texttt{'quoted'} or \texttt{otherwise},
represents terminals, $\epsilon$ represents the empty sequence.

First we change the right-hand side of each production to be a regular
expression instead of a sequence (i.e., our grammars are in Extended
Backus-Naur Form, or EBNF) and add (unique) labels to each production.
We use parentheses for grouping, $+$ for union, and $^{*}$ for Kleene
star.
...
\begin{center}
  \begin{tabular}{@{}l@{\quad$->$\quad}c@{ $:$\quad}l@{}}
    $E$ & $a$ & $E$ \texttt{'+'} $E$ \\
    $E$ & $m$ & $E$ \texttt{'*'} $E$ \\
    $E$ & $l$ & \texttt{'['} ($E$ (\texttt{','} $E$)$^{*}$ $+$ $\epsilon$) \texttt{']'} \\
    $E$ & $x$ & \texttt{'let'} \texttt{Ident} \texttt{'='} $E$ \texttt{'in'} $E$ \\
    $E$ & $n$ & \texttt{Int} \\
  \end{tabular}
\end{center}

\noindent Both changes serve to shape our ASTs. For example,
consider the string \texttt{'[1,2]'} with EBNF (left) and without (right):
...
\begin{center}
  \begin{tabular}{c@{\qquad}c}
    \begin{tikzpicture}
      ...
      \tikzset{edge from parent/.style={draw,edge from parent path={(\tikzparentnode.south) -- +(0,-4pt) -| (\tikzchildnode)}}}
      \Tree [.{$l$}
        \texttt{[}
          [.{$n$} \texttt{1} ]
          \texttt{,}
...
\noindent The extra internal nodes in the right tree would present
problems for disambiguation later.

Next we add grouping parentheses in the form of a pair of terminals
per non-terminal. Syntactically speaking these are equivalent with
adding a production $E \rightarrow \texttt{'('} E \texttt{')'}$,
except they are discarded when constructing the AST. Note the
difference between $()$ and \texttt{'('}\texttt{')'}, the former is
grouping in a regular expression, the latter is a sequence of two
terminals. We also allow discarding terminals that carry no useful
semantic information once the production has been identified. Since
most terminals do \emph{not} carry useful information we denote the
ones to be kept by underlining them:

\begin{center}
  \begin{tabular}{@{}l@{\quad$->$\quad}c@{ $:$\quad}l@{}}
    \multicolumn{3}{l}{Productions} \\
    \midrule
    $E$ & $a$ & $E$ \texttt{'+'} $E$ \\
    $E$ & $m$ & $E$ \texttt{'*'} $E$ \\
    $E$ & $l$ & \texttt{'['} ($E$ (\texttt{','} $E$)$^{*}$ $+$ $\epsilon$) \texttt{']'} \\
    $E$ & $x$ & \texttt{'let'} \underline{\texttt{Ident}} \texttt{'='} $E$ \texttt{'in'} $E$ \\
    $E$ & $n$ & \underline{\texttt{Int}} \\
  \end{tabular}
  \begin{tabular}{c} % TODO: figure out how to top align these two
    Grouping \\
    \midrule
    \texttt{'('}$E$\texttt{')'} \\
  \end{tabular}
...
\noindent We can thus use grouping parentheses and discarded terminals
for disambiguation. The former is often used for operator expressions,
while the latter can be used to, e.g., disambiguate with optional
semicolons. For example, the strings \texttt{'(1+2)*3'} and
\texttt{'[1,2]'} produce these ASTs:

\begin{center}
  \begin{tabular}{c@{\qquad}c}
    \begin{tikzpicture}
      \tikzset{level 1+/.style={level distance=16pt}}
      \tikzset{frontier/.style={distance from root=48pt}}
    ...
    \begin{tikzpicture}
      \tikzset{level 1+/.style={level distance=16pt}}
      \tikzset{frontier/.style={distance from root=32pt}}
      \tikzset{edge from parent/.style={draw,edge from parent path={(\tikzparentnode.south) -- +(0,-2pt) -| (\tikzchildnode)}}}
      \Tree [.{$l$}
        [.{$n$} \texttt{1} ]
        [.{$n$} \texttt{2} ] ]
...
\noindent Note that most terminals have been discarded from the
ASTs. Note also that we need to keep track of where each node came
from, i.e., which terminal or non-terminal in the grammar it
represents. For example, the $a$ node in the left AST came from the
first $E$ in the right-hand side of the $m$ production. This could,
e.g., be achieved by attaching unique labels to each non-terminal and
kept terminal in a right-hand side, then additionally annotating each
AST node with the corresponding label. However, in most examples this
connection will be obvious, thus we omit these to avoid cluttering the
presentation, and clarify the connection in text if necessary.

Finally we consider precedence and associativity. A common approach is
to create a \emph{precedence ladder} by splitting each non-terminal
into several non-terminals, one per precedence level. Explicitly doing
this is undesireable in our setting, for two reasons:

\begin{enumerate}
\item It requires total precedence, but we wish to allow, e.g.,
  leaving the precedence between operators from different languages
  undefined.
\item Grouping parentheses as described above recurse to the
  \emph{same} non-terminal, which would then not contain productions
  representing lower precedence operators.
\end{enumerate}

\noindent The former suggests that we cannot use precedence ladders
unchanged, and the latter suggests that we need some explicit support
in our grammars; we cannot simply rewrite the grammar without breaking
grouping parentheses.

We thus adopt an approach that coincides with precedence ladders if
precedence is total and transitive, but allows the absence of both
these properties. To simplify later analysis we treat precedence and
associativity as user-facing conveniences to be translated to a
simpler form. We describe the simpler form first, then the
translation, then illustrate both with examples.

We introduce \emph{marks}, annotations on the non-terminals appearing
on the right-hand side of a production. Each mark is a set of
production labels denoting the productions that may \emph{not} appear
in that position. For example, $E_{\{a, x\}}$ may parse as neither the
production labelled $a$, nor the production labelled $x$. It may,
however, parse as a pair of grouping parentheses that contain either of
these productions. We write $E_\emptyset$ as $E$ to reduce clutter.

Translation is then a process of examining the relative precedence of
every pair of productions and the associativity of each production and
adding marks as appropriate. For example, if $E -> m: E \texttt{'*'}
E$ has higher precedence than $E -> a: E \texttt{'+'} E$ we update $m$
to $E -> m: E_{\{a\}} \texttt{'*'} E_{\{a\}}$. If $m$ is additionally
left-associative we update it to $E -> m: E_{\{a\}} \texttt{'*'}
E_{\{a, m\}}$.

% TODO: figure out how to lay these out nicely. The first and third subfigure should fit vertically and be roughly the same height as the second.
\begin{figure*}[t]
  \subcaptionbox{The completed running example.\label{fig:grammar-example}}[.48\textwidth]{
    \begin{tabular}{@{}l@{\quad$->$\quad}c@{ $:$\quad}l@{}}
      \multicolumn{3}{l}{Productions} \\
      \midrule
      $E$ & $a$ & $E_{\{x\}}$ \texttt{'+'} $E_{\{x,a\}}$ \\
      $E$ & $m$ & $E_{\{x,a\}}$ \texttt{'*'} $E_{\{x,a,m\}}$ \\
      $E$ & $l$ & \texttt{'['} ($E$ (\texttt{','} $E$)$^{*}$ $+$ $\epsilon$) \texttt{']'} \\
      $E$ & $x$ & \texttt{'let'} \underline{\texttt{Ident}} \texttt{'='} $E$ \texttt{'in'} $E$ \\
      $E$ & $n$ & \underline{\texttt{Int}} \\
    \end{tabular}
  ...
  \subcaptionbox{The running example, translated to EBNF.\label{fig:grammar-example-ebnf}}[.48\textwidth]{
    \begin{tabular}{@{}l@{\quad$->$\quad}l@{}}
      $E$ & $E_{\{x\}}$ \texttt{'+'} $E_{\{x,a\}}$ \\
      $E$ & $E_{\{x,a\}}$ \texttt{'*'} $E_{\{x,a,m\}}$ \\
      $E$ & \texttt{'['} ($E$ (\texttt{','} $E$)$^{*}$ $+$ $\epsilon$) \texttt{']'} \\
      $E$ & \texttt{'let'} \texttt{Ident} \texttt{'='} $E$ \texttt{'in'} $E$ \\
      $E$ & \texttt{Int} \\
      $E$ & \texttt{'('} $E$ \texttt{')'} \\
      \addlinespace
      $E_{\{x\}}$ & $E_{\{x\}}$ \texttt{'+'} $E_{\{x,a\}}$ \\
      $E_{\{x\}}$ & $E_{\{x,a\}}$ \texttt{'*'} $E_{\{x,a,m\}}$ \\
      $E_{\{x\}}$ & \texttt{'['} ($E$ (\texttt{','} $E$)$^{*}$ $+$ $\epsilon$) \texttt{']'} \\
      $E_{\{x\}}$ & \texttt{Int} \\
      $E_{\{x\}}$ & \texttt{'('} $E$ \texttt{')'} \\
      \addlinespace
      $E_{\{x,a\}}$ & $E_{\{x,a\}}$ \texttt{'*'} $E_{\{x,a,m\}}$ \\
      $E_{\{x,a\}}$ & \texttt{'['} ($E$ (\texttt{','} $E$)$^{*}$ $+$ $\epsilon$) \texttt{']'} \\
      $E_{\{x,a\}}$ & \texttt{Int} \\
      $E_{\{x,a\}}$ & \texttt{'('} $E$ \texttt{')'} \\
      \addlinespace
      $E_{\{x,a,m\}}$ & \texttt{'['} ($E$ (\texttt{','} $E$)$^{*}$ $+$ $\epsilon$) \texttt{']'} \\
      $E_{\{x,a,m\}}$ & \texttt{Int} \\
      $E_{\{x,a,m\}}$ & \texttt{'('} $E$ \texttt{')'} \\
    \end{tabular}
  }
  \subcaptionbox{The running example, written in EBNF with a precedence ladder.\label{fig:grammar-example-ebnf-ladder}}[.48\textwidth]{
    \begin{tabular}{@{}l@{\quad$->$\quad}l@{}}
      $E$ & \texttt{'let'} \texttt{Ident} \texttt{'='} $E$ \texttt{'in'} $E$ \\
      $E$ & $E_1$ \\
      \addlinespace
      ...
      $E_2$ & $E_3$ \\
      \addlinespace
      $E_3$ & \texttt{'['} ($E$ (\texttt{','} $E$)$^{*}$ $+$ $\epsilon$) \texttt{']'} \\
      $E_3$ & \texttt{Int} \\
      $E_3$ & \texttt{'('} $E$ \texttt{')'} \\
    \end{tabular}
  }
  \caption{Completed grammars of the running example.}
\end{figure*}

% TODO: consistency between ``left associative'' and ``left-associative''
Applying the usual precedences to our running example (let $<$
addition $<$ multiplication) as well as associativities
(left-associative multiplication and addition) yields the grammar in
Figure~\ref{fig:grammar-example}\footnote{The observant reader may
  notice that this grammar no longer recognizes, e.g., \texttt{1 + let
    x = 2 in 3}. We discuss this in Section~\ref{sec:low-prec-unary}}.
Note that production $a$ has marks containing $x$ due to precedence
and $a$ due to associativity, and correspondingly $m$ has marks
containing both $x$ and $a$ due to precedence, and $m$ due to
associativity.

To illustrate the concrete syntax in more familiar terms we also show
a translation to normal EBNF grammars in
Figure~\ref{fig:grammar-example-ebnf}. The translation is done by
generating a new non-terminal per uniquely marked non-terminal,
duplicating all productions that are \emph{not} in the mark, and
adding grouping parentheses. This can be compared with
Figure~\ref{fig:grammar-example-ebnf-ladder} which is the same
language, but written more directly as a precedence ladder. Note that
Figure~\ref{fig:grammar-example-ebnf} is equivalent with
Figure~\ref{fig:grammar-example-ebnf-ladder} after inlining all
passthrough productions (e.g. $E -> E_1$).

When describing grammar formalisms a natural question is that of
expressiveness: how expressive are the grammars we present here?
Technically speaking they retain the full expressiveness of
context-free languages; we can trivially translate a CFG by simply
generating unique labels for each production and retaining all
terminals. However, such a translation would benefit from none of our
work as no information is discarded during parsing. We explore the
implications of this in the evaluation, in
Section~\ref{sec:expressiveness}.

We are now ready to describe the language of an AST.

\section{The Language of an Abstract Syntax Tree}\label{sec:tree-language}

Central to our approach is the idea that a single AST can be written
in multiple ways. This means that an AST corresponds to a set of
written programs, i.e., a language. We thus discuss two kinds of
languages: those described by the grammars in
Section~\ref{sec:grammars}, and those consisting of programs that may
parse as a given AST. We refer to the latter as
\emph{AST-languages}. The grammars described in
Section~\ref{sec:grammars} are designed to keep these AST-languages in
the class of visibly pushdown languages
\cite{alurVisiblyPushdownLanguages2004} which have many useful closure
properties. This section informally describes the process of obtaining
an AST-language from an AST and a grammar. We follow the same process
of explanation as Section~\ref{sec:grammars}, this time beginning with
EBNF grammars, then handling each additional modification. Note that
we do not discuss precedence or associativity, we instead handle marks
directly. As a running example we consider the AST obtained by parsing
\texttt{[1, !(2?)]} using the following grammar:
...
\begin{center}
  \begin{tabular}{@{}l@{\quad$->$\quad}c@{ $:$\quad}l@{}}
    ...
    $E$ & $q$ & $E_{\{c\}}$ \texttt{'?'} \\
    $E$ & $c$ & \texttt{'!'} $E$ \\
    $E$ & $l$ & \texttt{'['} ($E$ (\texttt{','} $E$)$^{*}$ $+$ $\epsilon$) (\texttt{','} $+$ $\epsilon$) \texttt{']'} \\
    $E$ & $n$ & \underline{\texttt{Int}} \\
  \end{tabular}
  \begin{tabular}{c} % TODO: figure out how to top align these two
    Grouping \\
...
When parsing using a normal EBNF grammar, without discarding any
information, a given AST can only be parsed in exactly one way, namely
the flattening of the tree. In this case, the AST (which is then more
like a parse tree) looks as on the left (using $g$ for grouping
parentheses), and the AST-language is recognized by the pushdown
automaton on the right (though we have no need of the stack yet):
  ...
  \begin{tabular}{cc}
    \begin{tikzpicture}
      ...
      \Tree [.{$l$}
        ...
        \texttt{]} ]
    \end{tikzpicture} &
    \begin{tikzpicture}[->,>=Latex,shorten >=1pt,auto,node distance=1cm,transform shape]
      \tikzset{every state/.style={minimum size=12pt}}
      \node[state,initial] (1) {};
      \node[state] (2) [right of=1] {};
      \node[state] (3) [below of=2] {};
      \node[state] (4) [left of=3] {};
      \node[state] (5) [left of=4] {};
      \node[state] (6) [below of=5] {};
      \node[state] (7) [right of=6] {};
      \node[state] (8) [right of=7] {};
      \node[state] (9) [below of=8] {};
      \node[state,accepting] (10) [left of=9] {};

      \path (1) edge node{\texttt{[}} (2);
      \path (2) edge node{\texttt{1}} (3);
      \path (3) edge[above] node{\texttt{,}} (4);
      \path (4) edge[above] node{\texttt{!}} (5);
      \path (5) edge[left] node{\texttt{(}} (6);
      \path (6) edge node{\texttt{2}} (7);
      \path (7) edge node{\texttt{?}} (8);
      \path (8) edge node{\texttt{)}} (9);
      \path (9) edge[above] node{\texttt{]}} (10);
    \end{tikzpicture}
  \end{tabular}
\end{center}

\noindent When ASTs additionally discard some terminals the
AST-language is no longer merely a flattening of the tree; instead it
instead has to represent all possible discarded sequences of
terminals. In some cases this yields no difference, e.g., production
$q$ discards exactly one terminal \texttt{'?'}. In other cases there
is a small difference, e.g., the end of production $l$ may discard
either \texttt{',]'} or merely \texttt{']'}. Our running example thus
yields:
...
\begin{center}
  \begin{tabular}{c@{\qquad}c}
    ...
    \begin{tikzpicture}[->,>=Latex,shorten >=1pt,auto,node distance=1cm,transform shape]
      ...
      \node[state] (6) [below of=5] {};
      \node[state] (7) [left of=6] {};
      \node[state] (8) [left of=7] {};
      \node[state] (9) [left of=8] {};
      % TODO: perform the first line break one step later, then have only the accepting state on the lowest line.
      \node[state] (11) [left of=9] {};
      \node[state,accepting] (10) [below of=11] {};

      \path (1) edge node{\texttt{[}} (2);
      \path (2) edge node{\texttt{1}} (3);
      \path (3) edge node{\texttt{,}} (4);
      \path (4) edge node{\texttt{!}} (5);
      \path (5) edge node{\texttt{(}} (6);
      \path (6) edge[above] node{\texttt{2}} (7);
      \path (7) edge[above] node{\texttt{?}} (8);
      \path (8) edge[above] node{\texttt{)}} (9);
      \path (9) edge node{\texttt{]}} (10);
      \path (9) edge[above] node{\texttt{,}} (11);
      \path (11) edge[left] node{\texttt{]}} (10);
    \end{tikzpicture}
  \end{tabular}
\end{center}

% TODO: I question the clarity of this paragraph. The idea is simple, but I'm not sure I'm conveying it properly
\noindent In general, the discarded sequence of terminals between two
retained points in a production must be described by a regular
language since all non-terminals are retained. For example, the AST
obtained by parsing \texttt{'[]'} consists of a single node $l$ with
no children. Its AST-language is described by the regular expression
$\texttt{'['} (\texttt{','} + \epsilon) \texttt{']'}$ since the other
sub-expressions in the production contain non-terminals (which must be
retained) that are not present in the AST.

Next we take discarded grouping parentheses into account. The AST now
discards the $g$ node, and the automaton must allow inserting an
arbitrary (balanced) amount of parentheses around each node. We ensure
this by pushing to the stack when recognizing a \texttt{'('} and
popping on \texttt{')'}. To ensure that parentheses pair correctly
around each node we use a distinct stack symbol for each node. These
stack symbols are represented by letters below. To reduce clutter we
write $\circ \xrightarrow{+a} \circ$ as shorthand for an edge that
recognizes \texttt{'('} and pushes $a$ on the stack, while $\circ
\xrightarrow{-a} \circ$ correspondingly recognizes \texttt{')'} and
pops $a$ from the stack\footnote{This works for our example since we
  have only a single form of grouping parentheses, otherwise we would
  have to be explicit.}.
...
\begin{center}
  \begin{tabular}{c@{\qquad}c}
    \begin{tikzpicture}
      \tikzset{level 1+/.style={level distance=16pt}}
      \tikzset{frontier/.style={distance from root=64pt}}
      \tikzset{edge from parent/.style={draw,edge from parent path={(\tikzparentnode.south) -- +(0,-2pt) -| (\tikzchildnode)}}}
      \Tree [.{$l$}
        [.{$n$} \texttt{1} ]
        [.{$c$}
    ...
    \begin{tikzpicture}[->,>=Latex,shorten >=1pt,auto,node distance=1cm,transform shape]
      ...
      \node[state] (4) [right of=3] {};
      \node[state] (5) [right of=4] {};
      \node[state] (7) [below of=4] {};
      \node[state] (8) [left of=7] {};
      % TODO: perform the first line break one step later, then have only the accepting state on the lowest line.
      \node[state] (11) [below of=1] {};
      \node[state,accepting] (10) [below of=11] {};
      ...
      \path (3) edge node{\texttt{,}} (4);
      \path (4) edge node{\texttt{!}} (5);
      \path (5) edge node{\texttt{2}} (7);
      \path (7) edge[above] node{\texttt{?}} (8);
      \path (8) edge node{\texttt{]}} (10);
      \path (8) edge[above] node{\texttt{,}} (11);
      \path (11) edge[left] node{\texttt{]}} (10);

      \path (1) edge[loop above] node{$+a$} (1);
      \path (10) edge[loop left] node{$-a$} (1);

      \path (2) edge[loop above] node{$+b$} (2);
      \path (3) edge[loop above] node{$-b$} (3);

      \path (4) edge[loop above] node{$+c$} (4);
      \path (5) edge[loop above,align=left] node{$+d$\\$+e$} (5);
      \path (8) edge[loop below,align=left] node{$-c$\\$-d$} (8);
      \path (7) edge[loop below] node{$-e$} (8);
    \end{tikzpicture}
  \end{tabular}
\end{center}

\noindent Finally we account for marks. A mark has the effect of
forbidding a particular node from being a direct child without at
least one intermediate pair of grouping parentheses. The AST-language
must thus require at least one pair in such a location. In this case
we have only one relevant mark: $c$ may not be a direct child of
$q$. The final AST-language is thus:
...
\begin{center}
  \begin{tabular}{c@{\qquad}c}
    ...
    \begin{tikzpicture}[->,>=Latex,shorten >=1pt,auto,node distance=1cm,transform shape]
      ...
      \path (3) edge node{\texttt{,}} (4);
      \path (4) edge node{\texttt{!}} (5);
      \path (5) edge node{$+d$} (6);
      \path (6) edge[above] node{\texttt{2}} (7);
      \path (7) edge[above] node{\texttt{?}} (8);
      \path (8) edge[above] node{$-d$} (9);
      \path (9) edge node{\texttt{]}} (10);
      \path (9) edge[above] node{\texttt{,}} (11);
      \path (11) edge[left] node{\texttt{]}} (10);
...
\noindent Note that there are now two required transitions $\circ
\xrightarrow{+d} \circ$ and $\circ \xrightarrow{-d} \circ$, i.e.,
there is a required pair of parentheses around the $q$ node.

This automaton is clearly visibly pushdown: it pushes on \texttt{'('}
and pops on \texttt{')'}, and does not examine the stack on any other
terminal. However, to ensure that this holds in general we require two
additional restrictions on our grammars:

\begin{itemize}
\item The set of opening brackets and closing brackets must be disjoint.
\item No production may recognize an unbalanced sequence of terminals,
  i.e., each recognized opening bracket has exactly one later closing
  bracket.
\end{itemize}

\noindent Note that the latter requirement is slightly less
restrictive than it may seem: an opening bracket can be closed with
\emph{any} closing bracket, not just the one it was defined with.

We are now ready to start resolving ambiguities.

\section{Resolvable Ambiguity}\label{sec:resolvable-definition}

This section introduces our definition of \emph{resolvable ambiguity},
and then relates it to standard concepts in formal languages.
...
\subsection{Expressiveness of Our Grammars}\label{sec:expressiveness}

% TODO: Write this section properly once the evaluation is done.

Technically speaking the grammars presented in this paper retain the
full expressibility of context-free languages; we can trivially
translate a CFG by simply generating unique labels for each
production. However, such a translation would benefit from none of our
work as no information is discarded during parsing. This section thus
discusses the expressibility limitations we have encountered while
designing grammars that do discard information appropriately.

\subsubsection{Longest Match}

Some commonly occurring language constructs require longest match,
perhaps most commonly if-then-else, commonly referred to as the
``dangling else'' problem.

\subsubsection{Low Precedence Unary Operators}\label{sec:low-prec-unary}
It can change the recognized language in the presence of
low-precedence unary
operators~\cite{aasaPrecedencesSpecificationsImplementations1995}. For
example, using normal CFGs, a language containing addition,
let-expressions, and numeric literals could be written naively (left)
or with a precedence ladder (right):

\begin{center}
  \begin{tabular}{cc}
    \begin{tabular}{@{}l@{\quad}l@{}}
      $E$\quad$->$ & \texttt{'let'} \texttt{Ident} \\
      & \texttt{'='} $E$ \texttt{'in'} $E$ \\
      $E$\quad$->$ & $E$ \texttt{'+'} $E$ \\
      $E$\quad$->$ & \texttt{Int} \\
    \end{tabular} &
    \begin{tabular}{@{}l@{\quad}l@{}}
      $E_1$\quad$->$ & \texttt{'let'} \texttt{Ident} \\
      & \texttt{'='} $E_1$ \texttt{'in'} $E_1$ \\
      $E_1$\quad$->$ & $E_2$ \\
      \addlinespace
      $E_2$\quad$->$ & $E_2$ \texttt{'+'} $E_3$ \\
      $E_2$\quad$->$ & $E_3$ \\
      \addlinespace
      $E_3$\quad$->$ & \texttt{Int} \\
    \end{tabular} \\
...
\noindent The left language recognizes \texttt{1 + let x = 2 in 3}
