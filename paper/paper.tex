...
Adding marks to such a language preserves resolvability:
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
  ...
  \node[state] (3) [below of=1,xshift=-1.2cm] {$3$};
  \node[state] (4) [right of=3] {$4$};
  \node[state] (5) [right of=4] {$5$};

  \node[state] (6) [below of=1,yshift=-2cm] {$6$};
  \node[state] (7) [right of=6] {$7$};

  \node[state] (8) [below of=6] {$8$};
  \node[state] (9) [right of=8] {$9$};

  \node[state,initial] (s) [left of=1,xshift=-1cm] {$s$};
  \node[state,accepting] (f) [right of=2,xshift=1cm] {$f$};

  \path (1) edge node[left]{$'\pospl', +(1, 2)$} (3)
  (5) edge[right] node{$'\pospr', -(1, 2)$} (2);
  \path (1) edge[out=180,in=180,looseness=2,left] node{$'\pospl', +(1, 2)$} (6)
  (7) edge[out=0,in=0,looseness=2,right] node{$'\pospr', -(1, 2)$} (2);

  \path (3) edge node{$'+'$} (4);
  \path (4) edge[bend right,above,out=270,in=225] node[xshift=5mm]{$'\pospl', +(4, 5)$} (3)
  (5) edge[loop right,below] node[yshift=-3.5mm]{$'\pospr', -(4, 5)$} (5);
  \path (4) edge[left] node{$'\pospl', +(4, 5)$} (6)
  (7) edge node{$'\pospr', -(4, 5)$} (5);

  \path (6) edge[left] node{$'\pospl', +(6, 7)$} (8)
  (9) edge[right] node{$'\pospr', -(6, 7)$} (7);

  \path (8) edge[above] node{$'n'$} (9);

  \path (s) edge[bend left] node{$'\pospl', +(s, f)$} (1)
  (2) edge[bend left] node{$'\pospr', -(s, f)$} (f);

\end{tikzpicture}
\end{center}

\noindent We then combine these disparate DFAs to produce a single visibly pushdown automaton in three steps:
...
\noindent To make the figure less cluttered we omit $'\pospl'$ and $'\pospr'$, i.e., we abbreviate $p \xrightarrow{'\pospl', +(p',q')} q$ as $p \xrightarrow{+(p', q')}$ and $p \xrightarrow{'\pospr', -(p',q')} q$ as $p \xrightarrow{-(p', q')}$.
...
\item $\delta$ is defined by the following equations (unspecified cases produce $\emptyset$):
  ...
  \begin{array}{r@{\;=\;}l@{\;\mid\;}l}
    ...
    & \{ (s_{l_3}, (p, q, w))
    & p \in Q_{l_1}, \delta_{l_1}(p, l_2) = q, (l_2, l_3, w) \in T \} \\

    \delta(s, \text{``}\pospl\text{''}, \lambda)
    & \{ (s_{l_2}, (s, f, w))
    & \labelnt(l_1) = S, (l_1, l_2, w) \in T \} \\
  \end{array}
  $$
\end{itemize}
